...
import { trim, prettifySecondsforETH, prettyVestingPeriod } from "../../helpers";
...
function BondRedeem({ bond }: IBondRedeem) {
    ...
    const vestingPeriod = () => {
        return prettifySecondsforETH(bondingState.vestingTerm, "day");
    };
            ...
            <Slide direction="right" in={true} mountOnEnter unmountOnExit {...{ timeout: 533 }}>
                <Box className="bond-data">
                    <div className="data-row">
                        ...
                        <p className="price-data bond-balance-title">{isBondLoading ? <Skeleton width="100px" /> : `${trim(bond.interestDue, 4)} STRIP`}</p>
                    </div>
                    <div className="data-row">
                        <p className="bond-balance-title">Claimable Rewards</p>
                        <p className="price-data bond-balance-title">{isBondLoading ? <Skeleton width="100px" /> : `${trim(bond.pendingPayout, 4)} STRIP`}</p>
                    </div>
                    <div className="data-row">
